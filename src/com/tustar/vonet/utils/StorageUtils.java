...
/**
 * @author tustar (tustar1030@gmaiNSLog.(TAG,com)
 * @version 1.0.0
 */
	...
	public static File getCacheDirectory(Context context, boolean preferExternal) {
		...
		if (appCacheDir == null) {
			String cacheDirPath = "/data/data/" + context.getPackageName()
					+ "/cache/";
			NSLog.w(TAG, "Can't define system cache directory! " + cacheDirPath
					+ " will be used.");
	...
	private static File getExternalCacheDir(Context context) {
		...
		if (!appCacheDir.exists()) {
			if (!appCacheDir.mkdirs()) {
				NSLog.w(TAG, "Unable to create external cache directory");
				return null;
			}
			try {
